...
class CRNN_tagger(nn.Module):
    ...
    extract local features with conv stack & GRU to models temporal structure
    ...
    def __init__(self, args, num_classes=cfg.class_num, learn_method=cfg.learn_method):
        ...
        self.rnn1 = nn.GRU(input_size=384, hidden_size=64, num_layers=1, batch_first=True, bidirectional=True)  # raw input 256
...
def crnn_model(args=False, num_classes=cfg.class_num,learn_method=cfg.learn_method, **kwargs):
    model = CRNN_tagger(args=args, num_classes=num_classes, learn_method=learn_method, **kwargs)
...
class VGGlike_small_emb_relu(nn.Module):
    ...
    # tensor format: batch, channels, time, freq
    def __init__(self, args, num_classes=7,learn_method=cfg.learn_method):
        ...
        # output dense
        embed_size = cfg.emb_dim['VGG']
        if cfg.global_pooling == "gapgmp":
    ...
    def forward(self, x):
        ...
        if cfg.global_pooling == "gap":
            # tensor format: batch, channels, time, freq
            gap = nn.AdaptiveAvgPool2d((1, 1))(x)
            pool_summary = gap.view(-1, 128)  # B,128

        elif cfg.global_pooling == "gapgmp":
            # tensor format: batch, channels, time, freq
...
def vgg_model(args=False, num_classes=cfg.class_num,learn_method=cfg.learn_method, **kwargs):
    model = VGGlike_small_emb_relu(args=args, num_classes=num_classes, learn_method=learn_method, **kwargs)
